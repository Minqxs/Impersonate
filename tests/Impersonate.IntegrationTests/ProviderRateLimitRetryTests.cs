...
public sealed class ProviderRateLimitRetryTests
{
    ...
    {
        var handler=new SequenceHandler();var client=new HttpClient(handler){BaseAddress=new("https://api.openai.test/")};var adapter=new OpenAiProviderAdapter(client);
        ...
        Assert.Equal("ok",response.Content);Assert.Equal(2,response.SameModelRequestAttemptCount);Assert.Equal(1,response.RateLimitRetryCount);Assert.True(response.ProviderResetUsed);Assert.Equal(2,handler.Requests.Count);Assert.All(handler.Requests,x=>Assert.Contains("\"model\":\"gpt-4.1\"",x));Assert.Equal(handler.Requests[0],handler.Requests[1]);
    }
    ...
    public async Task OpenAi_quota_exhaustion_is_not_retried()
    {
        ...
        Assert.Equal("provider_quota_exhausted",ex.Code);Assert.False(ex.IsTransient);Assert.True(ex.Capacity!.QuotaExhausted);Assert.Equal(HttpStatusCode.TooManyRequests,ex.Capacity.StatusCode);Assert.DoesNotContain("redacted provider detail",ex.Message);Assert.Equal(1,handler.Calls);
    }
    ...
    public async Task Reset_longer_than_ceiling_is_not_waited_or_immediately_retried()
    {
        ...
        var ex=await Assert.ThrowsAsync<ProviderRequestException>(()=>Complete(adapter));
        Assert.Equal("provider_rate_limited",ex.Code);Assert.Equal(1,handler.Calls);
    }
    ...
    public async Task Repeated_rate_limits_are_bounded()
    {
        ...
        var adapter=new OpenAiProviderAdapter(new HttpClient(handler){BaseAddress=new("https://api.openai.test/")},options);
        await Assert.ThrowsAsync<ProviderRequestException>(()=>Complete(adapter));Assert.Equal(3,handler.Calls);
    }
    ...
    private sealed class SequenceHandler:HttpMessageHandler
    {
        ...
        {
            Requests.Add(await request.Content!.ReadAsStringAsync(cancellationToken));if(Requests.Count==1){var limited=new HttpResponseMessage(HttpStatusCode.TooManyRequests){Content=new StringContent("{\"error\":{\"type\":\"tokens\",\"code\":\"rate_limit_exceeded\",\"message\":\"temporary\"}}",Encoding.UTF8,"application/json")};limited.Headers.TryAddWithoutValidation("Retry-After","0.01");limited.Headers.TryAddWithoutValidation("x-ratelimit-reset-tokens","10ms");limited.Headers.TryAddWithoutValidation("x-request-id","req-limited");return limited;}return new(HttpStatusCode.OK){Content=new StringContent("{\"id\":\"req-ok\",\"choices\":[{\"message\":{\"content\":\"ok\"}}],\"usage\":{\"prompt_tokens\":1,\"completion_tokens\":1}}",Encoding.UTF8,"application/json")};
        }
    }
    ...
    private sealed class AlwaysLimitedHandler(string body,string retryAfter):HttpMessageHandler
    {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request,CancellationToken cancellationToken){Calls++;var response=new HttpResponseMessage(HttpStatusCode.TooManyRequests){Content=new StringContent(body,Encoding.UTF8,"application/json")};response.Headers.TryAddWithoutValidation("Retry-After",retryAfter);response.Headers.TryAddWithoutValidation("x-request-id","req-safe");return Task.FromResult(response);}
    }
}
