...
public sealed class CoreCoderReviewerLoopTests
{
    ...
    {
        var run=ExecutableRun(maximumRevisions:1);var task=run.Tasks.Single();var tools=new LoopTools();
        ...
        Assert.True(coded.Succeeded);Assert.Equal(1,tools.PatchApplications);Assert.False(string.IsNullOrWhiteSpace((await tools.GetDiffAsync(new("workspace"),default)).Output));
        first.CompleteAttempt(coded.Summary);run.MoveTaskToReview(first);
        var firstReviewer=Reviewer("ChangesRequested","Add validation");var firstReview=await firstReviewer.Agent.ReviewAsync(ReviewContext(run,first,coded,tools),default);
        Assert.Contains("actualPatch",firstReviewer.Adapter.Requests.Single().UserContent);Assert.Contains("sha256",firstReviewer.Adapter.Requests.Single().UserContent);
        var rejected=run.RecordReview(first,firstReview.Decision!.Value,firstReview.Summary,firstReview.Feedback);run.ClearExecutionClaim();
        Assert.Equal(PlannedTaskStatus.ChangesRequested,first.Status);Assert.True(rejected.IsCurrent);
        ...
        Assert.True(revised.Succeeded);Assert.Equal(2,tools.PatchApplications);revision.CompleteAttempt(revised.Summary);run.MoveTaskToReview(revision);
        var secondReviewer=Reviewer("Approved",null);var secondReview=await secondReviewer.Agent.ReviewAsync(ReviewContext(run,revision,revised,tools),default);
        var approved=run.RecordReview(revision,secondReview.Decision!.Value,secondReview.Summary,secondReview.Feedback);run.FinishApprovedTask(revision);

        Assert.False(rejected.IsCurrent);Assert.True(approved.IsCurrent);Assert.Equal(1,task.RevisionCount);Assert.Equal(2,task.Attempts.Count);
        Assert.Equal(PlannedTaskStatus.Approved,task.Status);Assert.Equal(PipelineRunStatus.ReadyForDelivery,run.Status);Assert.Equal(LoopStage.Committing,run.LoopRun.CurrentStage);
    }

    [Fact]
    public async Task Invalid_fake_reviewer_output_cannot_approve_or_hide_failed_task()
    {
        var run=ExecutableRun(maximumRevisions:0,continueOnFailure:false);var task=run.ClaimNextTask(Guid.NewGuid(),"worker",DateTimeOffset.UtcNow.AddMinutes(1));var tools=new LoopTools();
        var coded=await Coder(tools).ExecuteAsync(Context(run,task,null),default);task.CompleteAttempt(coded.Summary);run.MoveTaskToReview(task);
        var invalidReviewer=ReviewerRaw("{\"decision\":\"Approved\",\"summary\":\"looks good\",\"feedback\":null,\"findings\":[{\"severity\":\"Blocking\",\"message\":\"broken\",\"path\":null,\"line\":null}]}");var reviewed=await invalidReviewer.Agent.ReviewAsync(ReviewContext(run,task,coded,tools),default);
        Assert.False(reviewed.Succeeded);Assert.Equal("invalid_reviewer_output",reviewed.FailureCode);Assert.Empty(task.ReviewDecisions);Assert.NotEqual(PlannedTaskStatus.Approved,task.Status);
        run.ResolveExecutionFailure(task,$"{reviewed.FailureCode}: {reviewed.FailureMessage}");
        Assert.Equal(PlannedTaskStatus.Failed,task.Status);Assert.Equal(PipelineRunStatus.Failed,run.Status);Assert.Empty(task.ReviewDecisions);
    }
    ...
    private sealed class SequenceAdapter(IReadOnlyList<string> responses):IAiProviderAdapter
    {
        ...
        public Task<LanguageModelResponse> CompleteAsync(ProviderConnectionContext connection,RoutedModel model,LanguageModelRequest request,CancellationToken cancellationToken){Requests.Add(request);return Task.FromResult(new LanguageModelResponse(responses[index++],$"request-{index}",10,5));}
    ...
    {
        public int PatchApplications{get;private set;}public string Diff=>PatchApplications==0?string.Empty:$"diff --git a/User.cs b/User.cs\n+revision {PatchApplications}\n";
        ...
        public Task<RepositoryToolResult> ApplyPatchAsync(WorkspaceReference workspace,string patch,CancellationToken ct){PatchApplications++;return Task.FromResult(new RepositoryToolResult(true,"applied"));}
