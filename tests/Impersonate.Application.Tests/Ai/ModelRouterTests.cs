...
public sealed class ModelRouterTests
{
    ...
    public void OpenAi_identity_rules_are_exact_and_ordered(string id,string canonical,string variant,bool known){var classifier=new ServiceCollection().AddApplication().BuildServiceProvider().GetRequiredService<IModelIdentityClassifier>();var result=classifier.Classify(ProviderType.OpenAI,id);Assert.Equal(canonical,result.CanonicalModel);Assert.Equal(variant,result.Variant.ToString());Assert.Equal(known,result.IsKnown);}
    [Fact] public void Feature_request_does_not_contaminate_small_child_task(){var profiler=new ServiceCollection().AddApplication().BuildServiceProvider().GetRequiredService<ITaskProfiler>();var profile=profiler.Profile(new(Guid.NewGuid(),null,AgentRole.Coder,"Add an IsActive boolean property with default true to the existing User entity.",TaskTitle:"Add IsActive",FeatureRequest:"Redesign authenticated security architecture",RepositoryLanguages:["C#"],AffectedAreas:["Domain"],ExpectedFileCount:1,ExpectedDiffSize:30));Assert.Equal(TaskComplexity.Simple,profile.Complexity);Assert.Equal(RiskLevel.Low,profile.Risk);Assert.False(profile.SecuritySensitive);Assert.False(profile.ArchitectureSensitive);}
    [Fact] public async Task OpenAi_flagship_mini_and_nano_have_distinct_capability_scores()
    {
        var project=Guid.NewGuid();var connection=AiProviderConnection.Create(ProviderType.OpenAI,"OpenAI");connection.Connected();
        ...
        var catalog=services.GetRequiredService<IModelCapabilityCatalog>();var flagshipProfile=catalog.Resolve(ProviderType.OpenAI,"gpt-4.1");var miniProfile=catalog.Resolve(ProviderType.OpenAI,"gpt-4.1-mini");var nanoProfile=catalog.Resolve(ProviderType.OpenAI,"gpt-4.1-nano");
