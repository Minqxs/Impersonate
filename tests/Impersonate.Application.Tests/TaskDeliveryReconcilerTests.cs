...
public sealed class TaskDeliveryReconcilerTests
{
    ...
    public async Task Open_pr_remains_awaiting_and_closed_pr_blocks()
    {
        ...
        Assert.Equal(TaskDeliveryStatus.AwaitingMerge, open.Delivery.Status); Assert.Null(open.Delivery.ClaimId);

        var closed = Fixture(PullRequestExternalState.Closed);
        await closed.Reconciler.ProcessOneAsync("worker", default);
        Assert.Equal(TaskDeliveryStatus.Blocked, closed.Delivery.Status); Assert.Equal("delivery_pull_request_closed", closed.Delivery.FailureCode);
    }
    ...
    public async Task Changed_head_blocks_and_transient_failure_keeps_checkpoint()
    {
        ...
        Assert.Equal(TaskDeliveryStatus.AwaitingMerge, transient.Delivery.Status); Assert.Null(transient.Delivery.ClaimId);
    }

    private static TestFixture Fixture(PullRequestExternalState state, string? failure = null, bool includeSkippedTask = false)
    {
        var run = PipelineRun.Create(Guid.NewGuid(), "feature"); run.StartPlanning(); var task = run.AddTask(1, "Task", "Work"); var skipped = includeSkippedTask ? run.AddTask(2, "Skipped", "Skipped work") : null; run.MarkReadyForExecution();
        typeof(PlannedTask).GetProperty(nameof(PlannedTask.Status))!.SetValue(task, PlannedTaskStatus.Approved);
        if (skipped is not null) typeof(PlannedTask).GetProperty(nameof(PlannedTask.Status))!.SetValue(skipped, PlannedTaskStatus.Skipped);
        ...
        delivery.StartPreparing(); delivery.RecordDeliveryBase("base"); delivery.RecordBranchPrepared("feature/task"); delivery.RecordPatchApplied(); delivery.RecordValidated(); delivery.RecordCommitted("commit"); delivery.RecordPushed("origin", "owner/repo", "feature/task", "commit"); delivery.RecordPullRequestOpen("GitHubMCP:test", "owner/repo", 1, "https://github.com/owner/repo/pull/1", "feature/task", "main", "commit", DateTimeOffset.UtcNow); delivery.AwaitMerge();
    ...
    {
        public Task<TaskDelivery?> ClaimNextReconciliationAsync(Guid id, string owner, DateTimeOffset at, DateTimeOffset expires, CancellationToken ct) { delivery.Claim(id, owner, expires, at); return Task.FromResult<TaskDelivery?>(delivery); }
