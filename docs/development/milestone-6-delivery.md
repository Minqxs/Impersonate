...
# Local delivery bootstrap

Local API and Worker delivery share the Development GitHub MCP configuration and startup readiness service. `scripts/local/start-impersonate.ps1` is the canonical entrypoint; IDE tasks call it instead of reproducing environment or process logic. The `/api/development/preflight` endpoint is read-only and verifies database connectivity, current migrations, writable Data Protection storage, Git availability, normalized repository identity, allowlisting, token availability, the official server identity, and the five required MCP tools. It performs no GitHub, Git, or AI mutation.
