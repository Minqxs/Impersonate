...
# Run integration topology

The current delivery topology is `default branch -> one run integration branch -> isolated task branches/internal pull requests -> one final aggregate pull request -> default branch`. `RunDelivery` owns aggregate identity and final-main readiness. `TaskDelivery` remains the per-approved-task audit boundary. Phase 1 deliberately exposes no external run-branch or final-pull-request mutation.
