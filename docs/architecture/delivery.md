...
The deterministic branch name includes the run identity, task sequence, normalized task title, and approved-patch hash. Delivery base, branch, validation summary, and commit identity are persisted at state boundaries. Leases prevent concurrent workers from claiming the same task; failures expose bounded codes and require explicit recovery. Local preparation does not push, create a provider branch, call GitHub, or open a pull request. Those external effects remain deferred to later phases.
