...
describe('planner completion UI', () => {
  ...
  it('disables retry when cumulative task AI budget is exhausted',async()=>{const invocation={id:'inv-1',sequence:1,agentRole:'Coder',provider:'OpenAI',model:'gpt-5',promptVersion:'coder-v1',inputTokenCount:80000,outputTokenCount:1000,responseType:'tool_calls',toolStepCount:3,successfulReadCount:3,successfulSearchCount:0,successfulPatchCount:0,providerRoundTripCount:4,consecutiveReadOnlyRounds:3,maximumSingleRequestInput:20000,structuredOutputRepairCount:0,noProgressCorrectionCount:1,paidProviderRequestCount:4,fallbackSequence:0,status:'Failed',failureCode:'coder_no_patch_progress',failureReason:'No patch progress.',startedAtUtc:'2026-07-20T00:00:00Z',completedAtUtc:'2026-07-20T00:01:00Z'};const exhausted={...run,status:'Failed',failureReason:'Task failed.',tasks:[{id:'one',sequence:1,title:'Add domain',description:'Add persistence.',acceptanceCriteria:['Notes persist.'],status:'Failed',revisionCount:0,maximumRevisionAttempts:3,dependsOnTaskIds:[],attempts:[{id:'attempt',attemptNumber:1,attemptType:'Initial',status:'Failed',toolStepCount:3,changedFiles:[],validationSummary:[],dependencyPatchCount:0,currentRevisionPatchApplied:false,incrementalPatchFileCount:0,invocations:[invocation]}],reviews:[]}],planningAttempts:[]};vi.mocked(fetch).mockImplementation(input=>String(input).endsWith('/timeline')?response([]):response(exhausted));renderDetail();expect(await screen.findByText(/task budget exhausted/)).toBeInTheDocument();expect(screen.getByRole('button',{name:'Retry task'})).toBeDisabled();});
