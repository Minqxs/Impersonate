...
async function request<T>(path: string, init?: RequestInit): Promise<T> { const response = await fetch(`${baseUrl}${path}`, { headers: { 'Content-Type': 'application/json', ...init?.headers }, ...init }); if (!response.ok) { const body = await response.json().catch(() => null) as { title?: string; errors?: Record<string, string[]> } | null; throw new Error(body?.errors ? Object.values(body.errors).flat().join(' ') : body?.title ?? `Request failed (${response.status}).`); } return response.json() as Promise<T>; }
export function listProjects(status?: ProjectStatus, search?: string) { const params = new URLSearchParams(); if (status) params.set('status', status); if (search) params.set('search', search); return request<Project[]>(`/api/projects${params.size ? `?${params}` : ''}`); }
...
export function getProjectHealth(id: string) { return request<Health>(`/api/projects/${id}/health`); }
