...
internal sealed class PlannerAgent(ILanguageModelClient client, IOptions<PlannerOptions> options, IEnumerable<Impersonate.Application.Ai.IAiProviderAdapter> adapters, Impersonate.Application.Ai.IProviderCredentialStore credentials) : IPlannerAgent
{
    ...
    public async Task<PlannerAgentResult> PlanAsync(PlannerAgentRequest request, CancellationToken ct)
    {
        ...
        {
            var credentialRead = await credentials.RetrieveAsync(connectionId, ct);
            if (credentialRead.Status != Impersonate.Application.Ai.ProviderCredentialReadStatus.Found)
                throw new Impersonate.Application.Ai.ProviderCredentialUnavailableException(credentialRead.SafeFailureCode!, credentialRead.SafeFailureMessage!);
            var credential = credentialRead.Credential!;
            ...
            response = await client.CompleteAsync(new(options.Value.Model, prompt, context, schema, options.Value.MaximumOutputTokens), ct);
    ...
    private static async Task<string> LoadPromptAsync(string version, CancellationToken ct)
    {
        if (version is not ("planner-v1" or "planner-v2"))
            throw new InvalidOperationException("Unsupported planner prompt version.");
        var assembly = typeof(PlannerAgent).Assembly;
