...
internal sealed class PlannerAgent(ILanguageModelClient client,IOptions<PlannerOptions> options,IEnumerable<Impersonate.Application.Ai.IAiProviderAdapter> adapters,Impersonate.Application.Ai.IProviderCredentialStore credentials):IPlannerAgent
{
 ...
 public async Task<PlannerAgentResult> PlanAsync(PlannerAgentRequest request,CancellationToken ct)
 {
  ...
  var context=JsonSerializer.Serialize(new{project=new{request.ProjectName,request.ProjectDescription,request.DefaultBranch},request.FeatureRequest,constraints=new{request.MaximumTasks,repositoryInspectionAvailable=request.RepositoryContext is not null,repositoryEvidenceRule="Every repositoryEvidence item must exactly equal one allowedRepositoryEvidencePaths value; use [] when no path supports the task."},repositoryContext=request.RepositoryContext is null?null:new{request.RepositoryContext.Tree,allowedRepositoryEvidencePaths=request.RepositoryContext.EvidencePaths.Order(StringComparer.OrdinalIgnoreCase),request.RepositoryContext.RelevantFiles,request.RepositoryContext.Languages,request.RepositoryContext.Frameworks,request.RepositoryContext.Layers,request.RepositoryContext.TestLocations,request.RepositoryContext.MigrationLocations,request.RepositoryContext.Summary},request.CorrectionContext});
