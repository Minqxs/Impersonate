...
internal sealed class PlanningRepositoryContextService(IRepositoryWorkspaceService workspaces,IRepositoryTools tools,IExecutionArtifactStore artifacts):IPlanningRepositoryContextService
{
 ...
 public async Task<PlanningRepositoryContextResult> BuildAsync(Guid projectId,Guid runId,string repositoryUrl,string defaultBranch,string featureRequest,CancellationToken ct)
 {
  ...
  try
  {
   var listed=await tools.ListFilesAsync(workspace,".",ct);if(!listed.Succeeded)return new(false,null,"planning_context_failed",listed.FailureMessage);var tree=listed.Output.Split('\n',StringSplitOptions.RemoveEmptyEntries|StringSplitOptions.TrimEntries).Take(MaximumTreeEntries).Select(Normalize).ToList();var selected=tree.Where(IsManifest).Concat(SelectRelevant(tree,featureRequest)).Distinct(StringComparer.OrdinalIgnoreCase).Take(MaximumFilesRead).ToList();var content=new Dictionary<string,string>();var total=0;
   foreach(var path in selected){var read=await tools.ReadFileAsync(workspace,path,ct);if(!read.Succeeded)continue;var value=read.Output.Length>MaximumFileBytes?read.Output[..MaximumFileBytes]:read.Output;var bytes=Encoding.UTF8.GetByteCount(value);if(total+bytes>MaximumTotalContextBytes)break;content[path]=value;total+=bytes;}
   var languages=DetectLanguages(tree);var frameworks=DetectFrameworks(content);var layers=DetectLocations(tree,["Domain","Application","Infrastructure","Api","frontend","src"]);var tests=tree.Where(x=>x.Contains("test",StringComparison.OrdinalIgnoreCase)).Select(Parent).Distinct().Take(30).ToList();var migrations=tree.Where(x=>x.Contains("migration",StringComparison.OrdinalIgnoreCase)).Select(Parent).Distinct().Take(30).ToList();var summary=$"Bounded snapshot: {tree.Count} paths, {content.Count} files read, {total} UTF-8 bytes; languages: {string.Join(", ",languages.DefaultIfEmpty("Unknown"))}; frameworks: {string.Join(", ",frameworks.DefaultIfEmpty("Unknown"))}.";var payload=JsonSerializer.Serialize(new{tree,relevantFiles=content,languages,frameworks,layers,testLocations=tests,migrationLocations=migrations,summary});var artifact=await artifacts.WriteTextAsync(new(projectId,runId,Guid.Empty,0),"planning-context.json",payload,"application/json",ct);var context=new PlanningRepositoryContext(tree,content.Keys.ToList(),languages,frameworks,layers,tests,migrations,summary,artifact.Reference,tree.ToHashSet(StringComparer.OrdinalIgnoreCase));return new(true,context,null,null);
  }
 ...
 private static IEnumerable<string> SelectRelevant(IReadOnlyList<string> tree,string request){var terms=request.Split([' ','-','_','/'],StringSplitOptions.RemoveEmptyEntries|StringSplitOptions.TrimEntries).Where(x=>x.Length>=4).Distinct(StringComparer.OrdinalIgnoreCase).Take(12).ToList();return tree.Where(path=>terms.Any(term=>path.Contains(term,StringComparison.OrdinalIgnoreCase))).Take(20);}
