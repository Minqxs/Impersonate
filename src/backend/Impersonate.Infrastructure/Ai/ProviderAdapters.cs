...
internal abstract class ProviderAdapterBase(HttpClient http) : IAiProviderAdapter
{
    ...
    {
        using var message=CompletionRequest(context,model,request);using var response=await Http.SendAsync(message,HttpCompletionOption.ResponseHeadersRead,ct);await EnsureSafeAsync(response,ct);var body=await response.Content.ReadAsStringAsync(ct);
        try{using var json=JsonDocument.Parse(body);return ParseCompletion(json.RootElement,response);}
        catch(Exception ex) when(ex is JsonException or InvalidOperationException or KeyNotFoundException){throw new ProviderRequestException("provider_invalid_response",$"The provider returned an invalid completion response. HTTP {(int)response.StatusCode}.",response.StatusCode,false);}
    }
...
internal class OpenAiProviderAdapter(HttpClient http) : ProviderAdapterBase(http)
{
    ...
    protected override IReadOnlyList<ProviderModel> ParseModels(JsonElement root) => root.GetProperty("data").EnumerateArray().Where(x=>IsLanguage(x.GetProperty("id").GetString()!)).Select(x=>{var id=x.GetProperty("id").GetString()!;var lifecycle=id.StartsWith("gpt-3.5",StringComparison.OrdinalIgnoreCase)?ModelLifecycleStatus.Deprecated:Lifecycle(id);var capabilities=ModelCapability.TextGeneration|ModelCapability.Reasoning|ModelCapability.ToolUse;if(SupportsJsonMode(id))capabilities|=ModelCapability.StructuredOutput;if(id.Contains("mini",StringComparison.OrdinalIgnoreCase)||id.Contains("nano",StringComparison.OrdinalIgnoreCase))capabilities|=ModelCapability.LowCost|ModelCapability.FastResponse;return new ProviderModel(id,id,null,lifecycle,capabilities,CapabilityMetadataSource.VersionedProviderMapping,null,null);}).ToList();
    ...
    protected override HttpRequestMessage CompletionRequest(ProviderConnectionContext c,RoutedModel m,LanguageModelRequest q){var r=Bearer(HttpMethod.Post,"v1/chat/completions",c.Credential.ApiKey);var body=new Dictionary<string,object?>{{"model",m.ProviderModelId},{"messages",new[]{new{role="system",content=q.SystemInstructions},new{role="user",content=q.UserContent}}}};if(SupportsJsonMode(m.ProviderModelId)){using var schema=JsonDocument.Parse(q.JsonSchema);body["response_format"]=new{type="json_schema",json_schema=new{name="planner_plan",strict=true,schema=schema.RootElement.Clone()}};}body[m.ProviderModelId.StartsWith("o",StringComparison.OrdinalIgnoreCase)?"max_completion_tokens":"max_tokens"]=q.MaximumOutputTokens;if(!m.ProviderModelId.StartsWith("o",StringComparison.OrdinalIgnoreCase))body["temperature"]=0;r.Content=JsonContent.Create(body);return r;}
