...
internal abstract class ProviderAdapterBase(HttpClient http) : IAiProviderAdapter
{
    ...
    protected static async Task EnsureSafeAsync(HttpResponseMessage response, CancellationToken ct) { if (!response.IsSuccessStatusCode) { await response.Content.ReadAsStringAsync(ct); throw new HttpRequestException($"Provider request failed with status {(int)response.StatusCode}.", null, response.StatusCode); } }
...
internal class OpenAiProviderAdapter(HttpClient http) : ProviderAdapterBase(http)
{
    ...
    protected override HttpRequestMessage CompletionRequest(ProviderConnectionContext c,RoutedModel m,LanguageModelRequest q){var r=Bearer(HttpMethod.Post,"v1/chat/completions",c.Credential.ApiKey);r.Content=JsonContent.Create(new{model=m.ProviderModelId,messages=new[]{new{role="system",content=q.SystemInstructions},new{role="user",content=q.UserContent}},response_format=new{type="json_object"}});return r;}
