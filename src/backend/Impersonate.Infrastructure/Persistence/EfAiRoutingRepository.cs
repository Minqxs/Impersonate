...
internal sealed class EfAiRoutingRepository(ImpersonateDbContext db) : IAiRoutingRepository
{
    ...
    public Task AddModelAsync(DiscoveredModel model, CancellationToken ct)
    {
        db.DiscoveredModels.Add(model);
        return Task.CompletedTask;
    }
    public Task RemoveConnectionAsync(AiProviderConnection connection, CancellationToken ct)
    {
        db.AiProviderConnections.Remove(connection);
        return Task.CompletedTask;
    }
    public async Task<ProjectAiRoutingPolicy> GetOrCreatePolicyAsync(Guid project, CancellationToken ct)
    {
        var policy = await db.ProjectAiRoutingPolicies.SingleOrDefaultAsync(x => x.ProjectId == project, ct);
        if (policy is null)
        {
            policy = ProjectAiRoutingPolicy.Create(project);
            db.ProjectAiRoutingPolicies.Add(policy);
        }
        return policy;
    }
    public Task AddDecisionAsync(ModelSelectionDecision decision, CancellationToken ct)
    {
        db.ModelSelectionDecisions.Add(decision);
        return Task.CompletedTask;
    }
