...
internal sealed class TaskDeliveryConfiguration : IEntityTypeConfiguration<TaskDelivery>
{
    public void Configure(EntityTypeBuilder<TaskDelivery> b)
    {
        ...
        b.Property(x => x.RemoteName).HasMaxLength(50);
        b.Property(x => x.RemoteRepository).HasMaxLength(300);
        b.Property(x => x.RemoteBranchName).HasMaxLength(250);
        b.Property(x => x.PushedCommitSha).HasMaxLength(64);
        ...
        b.HasIndex(x => new { x.RemoteRepository, x.RemoteBranchName }).IsUnique().HasFilter("[RemoteRepository] IS NOT NULL AND [RemoteBranchName] IS NOT NULL");
