...
internal sealed class EfPipelineRunRepository(ImpersonateDbContext db):IPipelineRunRepository
{
 ...
 public async Task<PipelineRun?> ClaimNextExecutionAsync(Guid claimId,string workerId,DateTimeOffset claimedAt,DateTimeOffset expiresAt,CancellationToken ct){await using var transaction=await db.Database.BeginTransactionAsync(System.Data.IsolationLevel.Serializable,ct);var run=await db.PipelineRuns.Include(x=>x.LoopRun).Include(x=>x.Tasks).ThenInclude(x=>x.Attempts).Include(x=>x.Tasks).ThenInclude(x=>x.ReviewDecisions).Include(x=>x.Events).Where(x=>x.Status==PipelineRunStatus.Executing&&(x.ExecutionClaimExpiresAtUtc==null||x.ExecutionClaimExpiresAtUtc<=claimedAt)).OrderBy(x=>x.CreatedAtUtc).FirstOrDefaultAsync(ct);if(run is null){await transaction.CommitAsync(ct);return null;}try{run.ClaimNextTask(claimId,workerId,expiresAt,claimedAt);await db.SaveChangesAsync(ct);await transaction.CommitAsync(ct);return run;}catch(DbUpdateConcurrencyException){await transaction.RollbackAsync(ct);db.ChangeTracker.Clear();return null;}}
