...
internal sealed class EfPipelineRunRepository(ImpersonateDbContext db) : IPipelineRunRepository
{
    ...
    public Task<PipelineRun?> GetAsync(Guid projectId, Guid runId, CancellationToken ct) => db.PipelineRuns.AsSplitQuery().Include(x => x.LoopRun).Include(x => x.Tasks).ThenInclude(x => x.Attempts).Include(x => x.Tasks).ThenInclude(x => x.ReviewDecisions).Include(x => x.Deliveries).Include(x => x.RunDelivery).Include(x => x.Events).SingleOrDefaultAsync(x => x.ProjectId == projectId && x.Id == runId, ct);
    ...
    {
        var q = db.PipelineRuns.AsNoTracking().AsSplitQuery().Include(x => x.LoopRun).Include(x => x.Tasks).ThenInclude(x => x.Attempts).Include(x => x.Tasks).ThenInclude(x => x.ReviewDecisions).Include(x => x.Deliveries).Include(x => x.RunDelivery).Where(x => x.ProjectId == projectId);
    ...
    public async Task DeleteAsync(Guid projectId, Guid runId, CancellationToken ct)
    {
        ...
        await db.RunDeliveries.Where(x => x.ProjectId == projectId && x.PipelineRunId == runId).ExecuteDeleteAsync(ct);
