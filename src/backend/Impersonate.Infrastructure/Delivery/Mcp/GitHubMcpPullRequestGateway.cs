...
internal sealed class GitHubMcpPullRequestGateway(IProjectRepository projects, IGitHubMcpClient mcp, IOptions<GitHubMcpOptions> configured) : IPullRequestGateway
{
    ...
    {
        if (string.IsNullOrWhiteSpace(mcp.ServerIdentity) || mcp.ServerIdentity.Length > 35 || mcp.ServerIdentity.Any(x => !char.IsAsciiLetterOrDigit(x) && x is not ('-' or '_'))) return Fail("github_mcp_server_not_allowed", "GitHub MCP server identity is invalid.");
        if (delivery.Status != TaskDeliveryStatus.Pushed || string.IsNullOrWhiteSpace(delivery.RemoteRepository) || string.IsNullOrWhiteSpace(delivery.RemoteBranchName) || string.IsNullOrWhiteSpace(delivery.PushedCommitSha)) return Fail("delivery_pull_request_state_invalid", "Only a verified pushed delivery can open a pull request.");
        if (!string.Equals(delivery.PushedCommitSha, delivery.CommitSha, StringComparison.OrdinalIgnoreCase)) return Fail("delivery_pull_request_head_changed", "Pushed commit no longer matches the approved delivery commit.");
        ...
        if (repository is null || !string.Equals(repository, delivery.RemoteRepository, StringComparison.OrdinalIgnoreCase) || !options.AllowedRepositories.Contains(repository, StringComparer.OrdinalIgnoreCase)) return Fail("github_mcp_repository_not_allowed", "Repository is not allowed for GitHub MCP delivery.");
        var parts = repository.Split('/');
        try
        {
            var existing = await FindAsync(parts[0], parts[1], project.DefaultBranch, delivery.RemoteBranchName, ct);
            if (existing is not null) return Match(existing, delivery, repository, project.DefaultBranch);
            JsonElement created;
            try
            {
                created = await mcp.CallToolAsync("create_pull_request", new { owner = parts[0], repo = parts[1], title = handoff.Title, head = delivery.RemoteBranchName, @base = project.DefaultBranch, body = Body(delivery, handoff), draft = options.DraftPullRequests, maintainer_can_modify = true }, ct);
            }
            ...
            var exact = await mcp.CallToolAsync("pull_request_read", new { method = "get", owner = parts[0], repo = parts[1], pullNumber = number }, ct);
    ...
    {
        if (!string.Equals(pull.HeadSha, delivery.CommitSha, StringComparison.OrdinalIgnoreCase)) return Fail("delivery_pull_request_head_changed", "Pull-request head does not match the approved delivery commit.");
        if (!string.Equals(pull.BaseBranch, baseBranch, StringComparison.Ordinal)) return Fail("delivery_pull_request_base_changed", "Pull-request base does not match the configured default branch.");
        if (pull.Merged || string.Equals(pull.State, "closed", StringComparison.OrdinalIgnoreCase)) return Fail("delivery_pull_request_closed", "A matching pull request is already closed.");
        if (!Uri.TryCreate(pull.Url, UriKind.Absolute, out var url) || url.Scheme != Uri.UriSchemeHttps || !string.Equals(url.Host, "github.com", StringComparison.OrdinalIgnoreCase)) return Fail("github_mcp_malformed_response", "GitHub MCP returned an unsafe pull-request URL.");
    ...
    private static string Body(TaskDelivery delivery, ApprovedTaskHandoff handoff)
    {
        ...
        foreach (var item in handoff.AcceptanceCriteria) text.AppendLine($"- {item}");
        text.AppendLine().AppendLine("Changed files:"); foreach (var file in handoff.ChangedFiles) text.AppendLine($"- `{file}`");
        ...
        if (handoff.DependencyTaskIds.Count > 0) text.AppendLine($"Dependencies: {string.Join(", ", handoff.DependencyTaskIds.Select(x => $"`{x}`"))}");
        return text.ToString();
    }
    private static IEnumerable<JsonElement> Items(JsonElement value)
    {
        if (value.ValueKind == JsonValueKind.Array) return value.EnumerateArray().Select(x => x.Clone()).ToArray();
        foreach (var name in new[] { "pull_requests", "pullRequests", "items", "result" }) if (value.ValueKind == JsonValueKind.Object && value.TryGetProperty(name, out var items) && items.ValueKind == JsonValueKind.Array) return items.EnumerateArray().Select(x => x.Clone()).ToArray();
        if (value.ValueKind == JsonValueKind.Object) throw new InvalidOperationException("github_mcp_malformed_response");
        return [];
    }
    private static PullRequest Parse(JsonElement value)
    {
        if (value.ValueKind == JsonValueKind.Object && value.TryGetProperty("pullRequest", out var nested)) value = nested;
