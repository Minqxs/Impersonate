...
internal sealed class LocalOfficialGitHubMcpClient(IOptions<GitHubMcpOptions> configured) : IGitHubMcpClient
{
    ...
    public async Task<JsonElement> CallToolAsync(string tool, object arguments, CancellationToken ct)
    {
        ...
        if (!string.Equals(file, "github-mcp-server", StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("github_mcp_server_not_allowed");
        ...
        if (!options.LocalArguments.SequenceEqual(["stdio"], StringComparer.Ordinal)) throw new InvalidOperationException("github_mcp_server_not_allowed");
        foreach (var argument in options.LocalArguments) start.ArgumentList.Add(argument);
        start.Environment.Clear();
        Copy("PATH"); Copy("SystemRoot"); Copy("WINDIR");
        var token = Environment.GetEnvironmentVariable(options.TokenEnvironmentVariable);
        if (!string.IsNullOrWhiteSpace(token)) start.Environment["GITHUB_PERSONAL_ACCESS_TOKEN"] = token;
        ...
        try
        {
            await WriteAsync(new { jsonrpc = "2.0", id = 1, method = "initialize", @params = new { protocolVersion = "2025-06-18", capabilities = new { }, clientInfo = new { name = "Impersonate", version = "1.0" } } }, timeout.Token);
            _ = McpJson.Result(await ReadResponseAsync(1, timeout.Token), 1);
            await WriteAsync(new { jsonrpc = "2.0", method = "notifications/initialized", @params = new { } }, timeout.Token);
            await WriteAsync(new { jsonrpc = "2.0", id = 2, method = "tools/call", @params = new { name = tool, arguments } }, timeout.Token);
        ...
        void Copy(string name) { var value = Environment.GetEnvironmentVariable(name); if (!string.IsNullOrWhiteSpace(value)) start.Environment[name] = value; }
        async Task WriteAsync(object value, CancellationToken token) { await process.StandardInput.WriteLineAsync(JsonSerializer.Serialize(value).AsMemory(), token); await process.StandardInput.FlushAsync(token); }
        async Task<string> ReadResponseAsync(long id, CancellationToken token)
        {
            while (true)
            {
                ...
                try { using var document = JsonDocument.Parse(line); if (document.RootElement.TryGetProperty("id", out var value) && value.ValueKind == JsonValueKind.Number && value.GetInt64() == id) return line; }
