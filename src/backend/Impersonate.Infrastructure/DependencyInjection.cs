...
public static class DependencyInjection
{
    ...
    public static IServiceCollection AddInfrastructure(this IServiceCollection services, IConfiguration configuration, IHostEnvironment environment)
    {
        ...
        services.AddOptions<GitHubMcpOptions>().Configure(x => { x.Tools = []; x.AllowedRepositories = []; }).BindConfiguration("Delivery:GitHubMcp").PostConfigure(x => { x.Transport = x.Transport.Trim(); x.ServerId = x.ServerId.Trim(); x.TokenEnvironmentVariable = x.TokenEnvironmentVariable.Trim(); x.Tools = x.Tools.Select(y => y.Trim()).Where(y => y.Length > 0).Distinct(StringComparer.Ordinal).ToArray(); x.AllowedRepositories = x.AllowedRepositories.Select(GitHubRepositoryIdentity.Normalize).Where(y => y is not null).Cast<string>().Distinct(StringComparer.OrdinalIgnoreCase).ToArray(); }).Validate(x => !x.Enabled || (x.TimeoutSeconds is >= 1 and <= 300 && (x.Transport == "Remote" || x.Transport == "Local") && x.ServerId == "github-official" && x.AllowedRepositories.Length > 0 && x.Tools.Length == 5 && x.Tools.Contains("list_pull_requests") && x.Tools.Contains("pull_request_read") && x.Tools.Contains("create_pull_request") && x.Tools.Contains("update_pull_request") && x.Tools.Contains("merge_pull_request")), "GitHub MCP delivery configuration is invalid.").ValidateOnStart();
        services.AddSingleton<GitHubMcpStartupReadiness>();
        services.AddSingleton<IHostedService>(x => x.GetRequiredService<GitHubMcpStartupReadiness>());
        services.AddSingleton<DevelopmentPreflightService>();
