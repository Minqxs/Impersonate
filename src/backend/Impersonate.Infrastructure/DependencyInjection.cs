...
public static class DependencyInjection
{
    ...
    public static IServiceCollection AddInfrastructure(this IServiceCollection services, IConfiguration configuration, IHostEnvironment environment)
    {
        ...
        services.AddOptions<GitHubMcpOptions>().BindConfiguration("Delivery:GitHubMcp").Validate(x => !x.Enabled || (x.TimeoutSeconds is >= 1 and <= 300 && (x.Transport == "Remote" || x.Transport == "Local") && x.Tools.Length == 3 && x.Tools.Contains("list_pull_requests") && x.Tools.Contains("pull_request_read") && x.Tools.Contains("create_pull_request")), "GitHub MCP delivery configuration is invalid.").ValidateOnStart();
