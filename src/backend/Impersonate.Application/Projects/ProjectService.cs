...
internal sealed class ProjectService(IProjectRepository repository) : IProjectService
{
    ...
    {
        var project = Project.Create(request.Name, request.Description, request.RepositoryUrl, request.DefaultBranch, request.Status);
    ...
    {
        var p = await repository.GetAsync(projectId, cancellationToken);
        if (p is null)
            return null;
        p.UpdateDetails(request.Name, request.Description, request.RepositoryUrl, request.DefaultBranch);
        await repository.SaveChangesAsync(cancellationToken);
        return Map(p);
    }
    public async Task<ProjectDto?> ChangeStatusAsync(Guid projectId, ProjectStatus status, CancellationToken cancellationToken)
    {
        var p = await repository.GetAsync(projectId, cancellationToken);
        if (p is null)
            return null;
    ...
    public async Task<ProjectHealthSummaryDto?> GetHealthAsync(Guid projectId, CancellationToken cancellationToken)
    {
        var p = await repository.GetAsync(projectId, cancellationToken);
        if (p is null)
            return null;
        return new(projectId, "Unknown", [new("RepositoryConfigured", "Healthy", "A repository URL is configured."), new("DefaultBranchConfigured", "Healthy", "A default branch is configured.")], DateTimeOffset.UtcNow);
    }
