...
internal sealed class DeterministicModelRouter(IAiRoutingRepository repository, ITaskProfiler profiler) : IModelRouter
{
    public async Task<ModelSelectionResult> SelectAsync(ModelSelectionRequest request, CancellationToken ct)
    {
        ...
        var models = (await repository.GetModelsAsync(null, ct)).Where(x => x.IsAvailable && connections.ContainsKey(x.ProviderConnectionId) && (policy.AllowPreviewModels || x.LifecycleStatus != ModelLifecycleStatus.Preview) && !(request.ExcludedModels?.Contains(x.Id) ?? false)).ToList();
    ...
    private static int Score(DiscoveredModel model, ModelCapability capabilities, TaskProfile profile, ProjectAiRoutingPolicy policy) { var score = 100; if (capabilities.HasFlag(ModelCapability.Reasoning)) score += profile.Complexity == TaskComplexity.High ? 40 : 15; if (capabilities.HasFlag(ModelCapability.StructuredOutput)) score += 20; if (model.ContextWindowSize >= profile.EstimatedContextSize * 2) score += 10; if (policy.PreferredProvider == model.ProviderType) score += 15; if (policy.CostPreference == RoutingPreference.Economy && capabilities.HasFlag(ModelCapability.LowCost)) score += 25; if (policy.LatencyPreference == RoutingPreference.Economy && capabilities.HasFlag(ModelCapability.FastResponse)) score += 25; return score; }
