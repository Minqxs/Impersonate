...
internal sealed class ExecutionOrderService : IExecutionOrderService
{
    ...
    public ExecutionOrderResult Order(IReadOnlyList<PlannerTask> tasks)
    {
        ...
        if (tasks.Any(x => (x.DependsOnSequences ?? []).Any(d => d == x.Sequence || !sequences.Contains(d))))
            return new(false, [], ["Dependencies must reference another task in the plan."]);
        var bySequence = tasks.ToDictionary(x => x.Sequence);
        var remaining = tasks.ToDictionary(x => x.Sequence, x => (x.DependsOnSequences ?? []).ToHashSet());
        var ordered = new List<PlannerTask>();
        while (remaining.Count > 0)
        {
            var ready = remaining.Where(x => x.Value.All(d => ordered.Any(o => o.Sequence == d))).Select(x => bySequence[x.Key]).OrderByDescending(x => x.EstablishesSharedContract).ThenBy(x => Conflict(x.ConflictRisk)).ThenBy(x => Layer(x.ChangeType, x.AffectedAreas ?? [])).ThenBy(x => x.Sequence).ThenBy(x => x.Title, StringComparer.Ordinal).ToList();
            if (ready.Count == 0)
                return new(false, [], ["Task dependency graph contains a cycle."]);
            var next = ready[0];
            ordered.Add(next);
            remaining.Remove(next.Sequence);
        }
    ...
    private static int Layer(string changeType, IReadOnlyList<string> areas)
    {
        var text = changeType + " " + string.Join(' ', areas);
        for (var i = 0; i < Layers.Length; i++)
            if (text.Contains(Layers[i], StringComparison.OrdinalIgnoreCase))
                return i;
