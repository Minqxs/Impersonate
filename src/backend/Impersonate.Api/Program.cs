...
{
    if(services.GetService<IPipelineRunRepository>() is null&&!legacyReadiness.Get().IsReady)return Results.Json(new ApiError("planner_configuration_unavailable",legacyReadiness.Get().Message),statusCode:503);
