 ...
 private async Task ProcessOneAsync(CancellationToken ct)
 {
  ...
  for(var number=prior+1;number<=options.Value.MaximumPlanningAttempts;number++)
  {
   ...
   try
   {
    ...
    if(errors.Count>0){correction=string.Join(" ",errors).Substring(0,Math.Min(1000,string.Join(" ",errors).Length));attempt.FailWithUsage(PlanningAttemptStatus.InvalidOutput,"invalid_output",correction,result.ProviderRequestId,result.InputTokenCount,result.OutputTokenCount);await db.SaveChangesAsync(ct);continue;}
