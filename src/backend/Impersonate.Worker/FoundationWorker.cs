...
public sealed class FoundationWorker(IServiceScopeFactory scopes,IOptions<PlannerOptions> options,ILogger<FoundationWorker> logger):BackgroundService
{
 ...
 private async Task ProcessOneAsync(CancellationToken ct)
 {
  ...
  for(var number=prior+1;number<=options.Value.MaximumPlanningAttempts;number++)
  {
   ...
   try
   {
    ProviderType? routedProvider=decision.ProviderConnectionId is null?null:Enum.Parse<ProviderType>(decision.Provider);var result=await agent.PlanAsync(new(project.Id,project.Name,project.Description,project.RepositoryUrl,project.DefaultBranch,run.FeatureRequest,options.Value.MaximumTasks,options.Value.PromptVersion,correction,decision.ProviderConnectionId,routedProvider,decision.Model,repositoryContext),ct);var plannedTasks=result.Plan.Tasks??[];logger.LogInformation("Planner response received for project {ProjectId}, pipeline {PipelineId}, attempt {Attempt}: canPlan={CanPlan}, taskCount={TaskCount}.",project.Id,run.Id,number,result.Plan.CanPlan,plannedTasks.Count);var errors=PlannerPlanValidator.Validate(result.Plan,options.Value.MaximumTasks,repositoryContext.EvidencePaths);
    if(errors.Count>0){correction=string.Join(" ",errors);correction=correction[..Math.Min(1000,correction.Length)];attempt.FailWithUsage(PlanningAttemptStatus.InvalidOutput,"invalid_output",correction,result.ProviderRequestId,result.InputTokenCount,result.OutputTokenCount);await db.SaveChangesAsync(ct);if(number<options.Value.MaximumPlanningAttempts&&decision.SelectionSource==ModelSelectionSource.AutomaticRouting&&decision.DiscoveredModelId is{} rejected){excludedModels.Add(rejected);var rerouted=await router.SelectAsync(new(project.Id,run.Id,AgentRole.Planner,run.FeatureRequest,ExcludedModels:excludedModels,FeatureRequest:run.FeatureRequest),ct);if(rerouted.Succeeded&&rerouted.Selection is{} choice){var replacement=ModelSelectionDecision.Create(project.Id,run.Id,AgentRole.Planner,choice.ConnectionId,choice.DiscoveredModelId,choice.ProviderType.ToString(),choice.ProviderModelId,choice.Source,choice.Score,JsonSerializer.Serialize(rerouted.Profile),$"Rerouted after invalid output from {decision.Model}. {choice.Explanation}",JsonSerializer.Serialize(rerouted.EligibleAlternatives.Take(3)),decision.Id,scoreBreakdown:JsonSerializer.Serialize(choice.ScoreBreakdown??[]),metadataVersion:choice.MetadataVersion);db.ModelSelectionDecisions.Add(replacement);await db.SaveChangesAsync(ct);decision=replacement;}}continue;}
    var ordered=orderService.Order(plannedTasks);if(!ordered.Succeeded){correction=string.Join(" ",ordered.Errors);attempt.FailWithUsage(PlanningAttemptStatus.InvalidOutput,"invalid_output",correction,result.ProviderRequestId,result.InputTokenCount,result.OutputTokenCount);await db.SaveChangesAsync(ct);continue;}await PersistSuccessfulPlanAsync(run.Id,attempt.Id,result,ordered.Tasks,ct);logger.LogInformation("Planning completed for project {ProjectId}, pipeline {PipelineId}, attempt {Attempt}.",project.Id,run.Id,number);return;
   }
