...
public sealed class FoundationWorker(IServiceScopeFactory scopes,IOptions<PlannerOptions> options,ILogger<FoundationWorker> logger):BackgroundService
{
 ...
 private async Task ProcessOneAsync(CancellationToken ct)
 {
  ...
  for(var number=prior+1;number<=options.Value.MaximumPlanningAttempts;number++)
  {
   ...
   try
   {
    ProviderType? routedProvider=decision.ProviderConnectionId is null?null:Enum.Parse<ProviderType>(decision.Provider);var result=await agent.PlanAsync(new(project.Id,project.Name,project.Description,project.RepositoryUrl,project.DefaultBranch,run.FeatureRequest,options.Value.MaximumTasks,options.Value.PromptVersion,correction,decision.ProviderConnectionId,routedProvider,decision.Model),ct);var plannedTasks=result.Plan.Tasks??[];logger.LogInformation("Planner response received for project {ProjectId}, pipeline {PipelineId}, attempt {Attempt}: canPlan={CanPlan}, taskCount={TaskCount}.",project.Id,run.Id,number,result.Plan.CanPlan,plannedTasks.Count);var errors=PlannerPlanValidator.Validate(result.Plan,options.Value.MaximumTasks);
    ...
    await PersistSuccessfulPlanAsync(run.Id,attempt.Id,result,plannedTasks,ct);logger.LogInformation("Planning completed for project {ProjectId}, pipeline {PipelineId}, attempt {Attempt}.",project.Id,run.Id,number);return;
   }
 ...
 private async Task PersistSuccessfulPlanAsync(Guid runId,Guid attemptId,PlannerAgentResult result,IReadOnlyList<PlannerTask> plannedTasks,CancellationToken ct)
 {
  ...
  if(!result.Plan.CanPlan)persistedRun.RequireClarification(result.Plan.FailureReason!,result.Plan.ClarifyingQuestion!);else{foreach(var task in plannedTasks.OrderBy(x=>x.Sequence))persistedRun.AddTask(task.Sequence,task.Title,task.Description,task.AcceptanceCriteria);persistedRun.MarkReadyForExecution();}
